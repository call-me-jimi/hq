...
def getDefaultPort( user, bias=0, add=0 ):
    ...
    defaultPort = 2**10 + sum( [ x[0]*2**x[1] for x in zip( [ ord(c) for c in user ], range(bias, len(user)+bias) ) ] ) % (2**16-2**10)
...
class SmartFormatter(argparse.HelpFormatter):
    def _split_lines(self, text, width):
        ...
        # split lines differently if the text starts with '**'
        if text.startswith('**'):
            newText = []
            [ newText.extend( textwrap.wrap(t,width=width) ) for t in text[2:].split('\n') ]
            return newText
        
        return argparse.HelpFormatter._split_lines(self, text, width)

        
## @brief ping a host in a thread
class hQPingHost(Thread):
    def __init__ (self,host):
        """! @brief constructor
