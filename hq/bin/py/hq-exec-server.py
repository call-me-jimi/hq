#!/usr/bin/env python
#
# hq/bin/py/hq-exec-server - start the hq exec server
#
...
if __name__ == '__main__':
    ...
    try:
        ...
        hQ = hQExecServer( args.hQPort )

        if args.daemonMode:
            if args.verboseMode:
                print "hQExecServer has been started as daemon on {host}:{port}".format(host=TMS.host, port=TMS.port)
            hQ.start() # run as deamon
        else:
            hQ.run() # run not as daemon
    except socket.error,msg:
        sys.stderr.write("\n")
        sys.stderr.write("hQExecServer Socket Error: %s\n" % msg.message)
        traceback.print_exc(file=sys.stdout)
    except SystemExit,msg:
        sys.stderr.write("\n")
        if msg.code == 0:
            sys.stderr.write("hQExecServer exit\n")
        else:
            sys.stderr.write("hQExecServer system error: %s\n" % msg.message)
    except:
        sys.stderr.write("\n")
        sys.stderr.write("hQExecServer error: %s\n" % sys.exc_info()[0])
        traceback.print_exc(file=sys.stdout)
        #raise
